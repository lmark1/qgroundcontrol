...
LinuxBuild {
    ...
    #- gstreamer assumed to be installed in /opt/gstreamer-1.0-android-universal-1.18.5/***
    contains(ANDROID_TARGET_ARCH, armeabi-v7a) {
        GST_ROOT = /opt/gstreamer-1.0-android-universal-1.18.5/armv7
    } else:contains(ANDROID_TARGET_ARCH, arm64-v8a) {
        GST_ROOT = /opt/gstreamer-1.0-android-universal-1.18.5/arm64
    } else:contains(ANDROID_TARGET_ARCH, x86_64) {
        GST_ROOT = /opt/gstreamer-1.0-android-universal-1.18.5/x86_64
    } else {
        message(Unknown ANDROID_TARGET_ARCH $$ANDROID_TARGET_ARCH)
        GST_ROOT = /opt/gstreamer-1.0-android-universal-1.18.5/x86
    }
    message(GST is $$GST_ROOT)
    exists($$GST_ROOT) {
